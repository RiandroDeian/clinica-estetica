...
const CORES = ["#c8a078","#b08060","#a07050","#d0a080","#c09070","#a08070","#b09080","#8a6f5a","#d4b896","#7a9080"];
...
export default function ProcedimentosPage() {
  ...
  const [form, setForm] = useState({ nome: "", cor: "#c8a078", duracao_minutos: 60, preco: "" });
  ...
  function abrirNovo() {
    setEditando(null);
    setForm({ nome: "", cor: "#c8a078", duracao_minutos: 60, preco: "" });
    setModalAberto(true);
  }

  function abrirEditar(p: Procedimento) {
    setEditando(p);
    setForm({ nome: p.nome, cor: p.cor, duracao_minutos: p.duracao_minutos, preco: p.preco?.toString() ?? "" });
    setModalAberto(true);
  }

  async function salvar() {
    setSalvando(true);
    const body = { ...form, preco: form.preco ? parseFloat(form.preco) : null, duracao_minutos: Number(form.duracao_minutos) };
    if (editando) {
