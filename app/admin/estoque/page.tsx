...
export default function EstoquePage() {
  ...
  async function salvar() {
    setSalvando(true);
    const payload = { ...form, quantidade: Number(form.quantidade), quantidade_minima: Number(form.quantidade_minima), custo_medio: form.custo_medio ? Number(form.custo_medio) : null };
    if (editando) {
                      ...
                      <tr key={item.id} style={{ borderBottom: i < itensFiltrados.length - 1 ? "1px solid var(--border-subtle)" : "none" }}>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex gap-2">
                            <button onClick={() => { setEditando(item); setForm({ nome: item.nome, categoria: item.categoria ?? "", quantidade: String(item.quantidade), unidade: item.unidade, quantidade_minima: String(item.quantidade_minima), custo_medio: String(item.custo_medio ?? ""), fornecedor: item.fornecedor ?? "", validade: item.validade ?? "" }); setModalAberto(true); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(8px)" }}>
          <div className="w-full max-w-sm rounded-3xl p-8" style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)" }}>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                <label className="text-xs uppercase tracking-widest block mb-2" style={{ color: "var(--text-secondary)" }}>Motivo</label>
