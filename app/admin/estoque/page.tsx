...
export default function EstoquePage() {
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-6 p-1 rounded-2xl" style={{ background: "#120d0d", border: "1px solid rgba(200,160,120,0.1)" }}>
        {[{ key: "itens", label: `Itens (${itens.length})` }, { key: "historico", label: `Historico (${historico.length})` }].map(a => (
          <button key={a.key} onClick={() => setAbaAtiva(a.key as any)}
            ...
            style={{ background: abaAtiva === a.key ? "rgba(200,160,120,0.15)" : "transparent", color: abaAtiva === a.key ? "#c8a078" : "#6b5a4e", border: abaAtiva === a.key ? "1px solid rgba(200,160,120,0.2)" : "1px solid transparent" }}>
            {a.label}
          </button>
        ))}
      </div>
      ...
      {carregando ? (
        ...
        itens.length === 0 ? (
          <div className="text-center py-20 rounded-3xl" style={{ background: "#120d0d", border: "1px solid rgba(200,160,120,0.1)" }}>
            ...
            <p className="text-sm" style={{ color: "#6b5a4e" }}>Estoque vazio</p>
          </div>
        ) : (
              ...
              <table className="w-full">
                ...
                <tbody>
                  {itens.map((item, i) => {
                    ...
                    return (
                      <tr key={item.id} style={{ borderBottom: i < itens.length - 1 ? "1px solid rgba(200,160,120,0.06)" : "none" }}>
                        <td className="px-5 py-4"><p className="text-sm font-semibold" style={{ color: "#e8d5c0" }}>{item.nome}</p></td>
                        ...
                        <td className="px-5 py-4 text-sm" style={{ color: "#6b5a4e" }}>{item.quantidade_minima} {item.unidade}</td>
                        ...
                        <td className="px-5 py-4 text-sm" style={{ color: item.validade && new Date(item.validade) < new Date() ? "#e87a7a" : "#6b5a4e" }}>
        ...
        historico.length === 0 ? (
          <div className="text-center py-20 rounded-3xl" style={{ background: "#120d0d", border: "1px solid rgba(200,160,120,0.1)" }}>
            <p className="text-4xl mb-4">📋</p>
            <p className="text-sm" style={{ color: "#6b5a4e" }}>Nenhuma movimentacao ainda</p>
          </div>
        ) : (
          <div className="rounded-3xl overflow-hidden" style={{ background: "#120d0d", border: "1px solid rgba(200,160,120,0.12)" }}>
            <div className="px-6 py-4" style={{ borderBottom: "1px solid rgba(200,160,120,0.1)" }}>
              <h2 className="text-xs uppercase tracking-widest" style={{ color: "#c8a078" }}>Historico de Movimentacoes</h2>
            </div>
                ...
                <div key={mov.id} className="flex items-center gap-4 px-6 py-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-semibold" style={{ color: "#e8d5c0" }}>{mov.estoque?.nome}</p>
      ...
      {modalMov && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(8px)" }}>
          <div className="w-full max-w-sm rounded-3xl p-8" style={{ background: "#120d0d", border: "1px solid rgba(200,160,120,0.2)" }}>
            ...
            <p className="text-sm mb-6" style={{ color: "#6b5a4e" }}>{modalMov.item.nome} - Atual: {modalMov.item.quantidade} {modalMov.item.unidade}</p>
            <div className="flex flex-col gap-4">
              <div>
                <label className="text-xs uppercase tracking-widest block mb-2" style={{ color: "#a89080" }}>Quantidade</label>
                <input type="number" value={qtdMov} onChange={e => setQtdMov(e.target.value)} placeholder="0"
                  className="w-full rounded-2xl px-4 py-3 text-sm outline-none"
                  style={{ background: "#0e0a0a", border: "1px solid rgba(200,160,120,0.15)", color: "#e8d5c0" }} />
              </div>
              <div>
                <label className="text-xs uppercase tracking-widest block mb-2" style={{ color: "#a89080" }}>Motivo</label>
