...
function StatsCard({ titulo, valor, icone, sub }: { titulo: string; valor: string | number; icone: string; sub?: string }) {
  ...
  return (
    <div className="rounded-3xl p-6 transition-all duration-500"
      style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", opacity: mounted ? 1 : 0, transform: mounted ? "translateY(0)" : "translateY(16px)" }}>
      <div className="w-10 h-10 rounded-2xl flex items-center justify-center text-lg mb-4" style={{ background: "var(--gold-bg)" }}>
        {icone}
      </div>
      <p className="text-3xl font-bold mb-1" style={{ color: "var(--gold)" }}>{valor}</p>
...
export default function DashboardPage() {
  ...
  function CardAgendamento({ ag }: { ag: Agendamento }) {
    return (
      <div className="flex items-center gap-4 p-4 rounded-2xl group"
        style={{ background: "var(--bg-input)", border: ag.sem_cadastro ? "1px solid rgba(251,191,36,0.2)" : "1px solid rgba(200,160,120,0.08)" }}>
        <div className="w-1 h-12 rounded-full flex-shrink-0" style={{ background: ag.procedimentos?.cor ?? "var(--gold)" }} />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="text-sm font-semibold truncate" style={{ color: "var(--text-primary)" }}>{nomePaciente(ag)}</p>
  ...
  return (
    <div>
      <div className="mb-8">
        <p className="text-xs uppercase tracking-widest mb-1 capitalize" style={{ color: "var(--gold)" }}>{hoje}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-3xl p-6" style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)" }}>
          <h2 className="text-sm font-semibold uppercase tracking-widest mb-5" style={{ color: "var(--gold)" }}>
            Agenda de Hoje
          </h2>
          {(data?.agendamentosHoje.length ?? 0) === 0 ? (
            <div className="text-center py-10">
              <p className="text-3xl mb-3">📋</p>
              <p className="text-sm" style={{ color: "var(--text-muted)" }}>Nenhum agendamento hoje</p>
            </div>
          ) : (
                ...
                return (
                  <div key={i}>
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-sm font-semibold" style={{ color: "var(--gold)" }}>{p.total}</span>
                    </div>
                    <div className="h-1.5 rounded-full" style={{ background: "var(--gold-bg)" }}>
                      <div className="h-1.5 rounded-full transition-all duration-700" style={{ width: `${pct}%`, background: "var(--gold)" }} />
                    </div>
                  </div>
                );
              })}
      ...
      <div className="mt-6 rounded-3xl p-6" style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)" }}>
        <h2 className="text-sm font-semibold uppercase tracking-widest mb-5" style={{ color: "var(--gold)" }}>
          Próximos Agendamentos
        </h2>
        {(data?.proximosAgendamentos.length ?? 0) === 0 ? (
          <div className="text-center py-8">
            <p className="text-3xl mb-3">🗓</p>
            <p className="text-sm" style={{ color: "var(--text-muted)" }}>Nenhum próximo agendamento</p>
          </div>
        ) : (
            ...
            {data?.proximosAgendamentos.map(ag => (
              <div key={ag.id} className="p-4 rounded-2xl group relative"
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs uppercase tracking-wider truncate" style={{ color: ag.procedimentos?.cor ?? "var(--gold)" }}>
                    {ag.procedimentos?.nome ?? "Procedimento"}
                  </span>
                </div>
                <p className="text-sm font-semibold mb-1" style={{ color: "var(--text-primary)" }}>{nomePaciente(ag)}</p>
